    ...
    m_network_style(networkStyle)
{
    ...
    {
        progressBar->setStyleSheet("QProgressBar { background-color: #2a2a2a; border: 1px solid #555; border-radius: 7px; padding: 1px; text-align: center; color: #e6e6e6; } QProgressBar::chunk { background: QLinearGradient(x1: 0, y1: 0, x2: 1, y2: 0, stop: 0 #FF4500, stop: 0.5 #FF8C00, stop: 1 #FFBF00); border-radius: 7px; margin: 0px; }");
    }
...
void BitcoinGUI::createActions()
{
    ...
    dashboardAction = new QAction(platformStyle->SingleColorIcon(":/icons/eye"), tr("&Dashboard"), this);
    dashboardAction->setStatusTip(tr("View real-time node monitoring and statistics"));
    dashboardAction->setToolTip(dashboardAction->statusTip());
    dashboardAction->setCheckable(true);
    dashboardAction->setShortcut(QKeySequence(QStringLiteral("Alt+5")));
    tabGroup->addAction(dashboardAction);

    sovereignControlsAction = new QAction(platformStyle->SingleColorIcon(":/icons/configure"), tr("So&vereign"), this);
    sovereignControlsAction->setStatusTip(tr("Sovereign Controls — configure your node policy settings"));
    sovereignControlsAction->setToolTip(sovereignControlsAction->statusTip());
    sovereignControlsAction->setCheckable(true);
    sovereignControlsAction->setShortcut(QKeySequence(QStringLiteral("Alt+6")));
    tabGroup->addAction(sovereignControlsAction);
    ...
    connect(dashboardAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(dashboardAction, &QAction::triggered, this, &BitcoinGUI::gotoDashboardPage);
    connect(sovereignControlsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sovereignControlsAction, &QAction::triggered, this, &BitcoinGUI::gotoSovereignControlsPage);
#endif // ENABLE_WALLET
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        toolbar->addAction(sovereignControlsAction);
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    sovereignControlsAction->setEnabled(enabled);
...
void BitcoinGUI::gotoSovereignControlsPage()
{
    sovereignControlsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSovereignControlsPage();
}
